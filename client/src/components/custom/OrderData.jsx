  ...
  return (
    <Card className="grid gap-2 p-5">
            ...
            products.map((product)=>(
               <div className='flex flex-col sm:flex-row justify-between items-end sm:items-center border p-3 rounded-lg bg-gray-100 dark:bg-zinc-900'>
                <div className='flex items-center gap-2'>
                  <img src={product?.images?.[0].url} alt={product?.id?.name} className='w-20 h-20 rounded-lg'/>
                  <div className='grid gap-1'>
                    ...
                    <p className='flex flex-col sm:flex-row sm:gap-2 text-gray-500 dark:text-customGray text-xs sm:text-sm my-0'>
                     <span>
                        Color: <span style={{backgroundColor: product?.color }}>{"white"}</span>
                     </span>
                ...
                <h2 className='text-md sm:text-xl font-bold flex items-center dark:text-customYellow'>
                    <DollarSign size={18}/>{product?.id.price}
                </h2>
        ...
        <div className='flex flex-col sm:flex-row justify-between sm:items-center'>
            <span>
                Ordered On: <span>{new Date(createdAt).toLocaleString()}</span>
            </span>
            <span className='hover:underline text-sm cursor-pointer flex items-center gap-1 dark:text-customYellow'>
                <ArrowDownToLine onClick={handleDownloadInvoice} size={10}/>
                Download Invoice
            </span>
        </div>
        <hr/>
