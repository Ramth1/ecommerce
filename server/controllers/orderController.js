...
const getAllOrders = async(req,res) =>{
    ...
    try {
        const orders = await Order.find().populate({
            path:"products.id",
            select:"name price category images address"
        }).populate({
            path:"userId",
            select:"name email "
        ...
        if(!orders){
            return res.status(404).json({success:false, message:"No orders to show"});
        }
...
module.exports = { getMetrics, getOrdersByUserId, getAllOrders, updateOrderStatus};